...
// simulation directories i.e. include, src, etc.)
{
  ...
  auto runType{static_cast<std::string>("original")};  // original, nogap, etc.
  ...
  //std::string energies[6] = {"0001", "0050", "1", "10", "50", "1000"};
  ...
  //for (auto const& i : energies) {
    ...
    // data file
  auto fileName{"ACE3_Data_" + runType + "_05" + /*i.c_str()*/ + ".root"};
    ...
    ht1->GetXaxis()->SetRangeUser(3600,3800);
    ...
    ht2->GetXaxis()->SetRangeUser(3600,3800);
    ...
    ht3->GetXaxis()->SetRangeUser(3600,3800);
